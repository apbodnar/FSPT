...
void main(void) {
  ...
  for(int i=0; i < NUM_BOUNCES; ++i){
    if(result.index < 0.0){
      ...
      indirectSamples[i] = envColor(ray.dir);
      break;
    }
    bounces++;
